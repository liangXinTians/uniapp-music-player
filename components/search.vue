		...
		<uni-nav-bar>
			<view class="search-box flex-box">
				<view class="top-search flex-box flex-item">
					...
					<input type="text" class="flex-item" v-model="searchTxt" :focus="isFocus" @input="inputSearch"
						@confirm="addSearchHistory" />
				</view>
			</view>
			<block slot="right">		
			...
			<template v-if="!searchTxt.trim().length">
				...
				<view class="history-list" v-if="historyList.length">
					<view class="title-bar flex-box">
						...
						<image class="icon" src="../static/image/search/3.png" mode="widthFix" @click="clearSearch">
						</image>
					</view>
